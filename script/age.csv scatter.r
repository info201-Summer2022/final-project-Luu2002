...
sca <- plot_ly(age, x = ~V8, y = ~V24, type = 'scatter', name = 'Both', mode = 'makers', marker = list(opacity = 1))
fig <- fig %>% add_trace(y = ~V27, name = 'High')
fig <- fig %>% add_trace(y = ~V30, name = 'Low')
fig <- fig %>% layout(xaxis = list(showgrid = FALSE), yaxis = list(title = "Age"), title = "Average Sucide Age", barmode = 'group')
fig
